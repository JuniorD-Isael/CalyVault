(function() {
    ...
    function ensureCalyStyles() {
        ...
        style.textContent = `
            ...
            @keyframes calySlideOutRight { 0% { transform: translateX(0); opacity: 1; max-height: 80px; margin-bottom: 0; padding: 16px 20px; } 50% { opacity: 0; transform: translateX(50px); } 100% { transform: translateX(100px); opacity: 0; max-height: 0; margin-bottom: 0; padding: 0; border: none; } }
            #caly-fab { position: fixed; bottom: 30px; right: 30px; width: 50px; height: 50px; background: linear-gradient(135deg, #8b5cf6, #6d28d9); border-radius: 50%; box-shadow: 0 0 20px rgba(139, 92, 246, 0.4); display: flex; align-items: center; justify-content: center; z-index: 99999; cursor: pointer; border: 2px solid rgba(255,255,255,0.1); transition: all 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275); color: white; }
            #caly-fab:hover { transform: scale(1.1) rotate(15deg); box-shadow: 0 0 30px rgba(139, 92, 246, 0.8); background: #a78bfa; }
            .caly-overlay { position: fixed; inset: 0; background: rgba(0, 0, 0, 0.85); backdrop-filter: blur(8px); z-index: 100000; display: flex; align-items: center; justify-content: center; animation: calyFadeIn 0.2s ease-out; }
            .caly-modal { background: linear-gradient(135deg, #13131a 0%, #1e1e2e 100%); border: 2px solid #8b5cf6; border-radius: 12px; width: 600px; max-width: 90vw; box-shadow: 0 20px 60px rgba(0,0,0,0.9), 0 0 0 1px rgba(139, 92, 246, 0.3); animation: calySlideUp 0.1s ease-out; color: #fff; font-family: "Motiva Sans", sans-serif; overflow: hidden; display: flex; flex-direction: column; }
            .caly-header { padding: 20px; background: linear-gradient(90deg, #2e1065, #13131a); border-bottom: 2px solid rgba(139, 92, 246, 0.3); display: flex; justify-content: space-between; align-items: center; }
            .caly-title { font-size: 22px; font-weight: 700; background: linear-gradient(135deg, #c4b5fd 0%, #8b5cf6 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; letter-spacing: 0.5px; }
            ...
            .caly-input { background: rgba(0,0,0,0.3); border: 1px solid #4c1d95; color: white; padding: 12px; border-radius: 6px; width: 100%; font-family: inherit; outline: none; transition: all 0.2s; font-size: 14px; box-sizing: border-box; }
            ...
            .caly-item { padding: 12px 20px; border-bottom: 1px solid rgba(255,255,255,0.05); display: flex; align-items: center; gap: 15px; transition: background 0.2s; position: relative; }
            .caly-item:hover { background: rgba(139, 92, 246, 0.08); }
            .caly-game-img { width: 90px; height: 42px; object-fit: cover; border-radius: 4px; box-shadow: 0 2px 5px rgba(0,0,0,0.5); background: #000; }
            .caly-no-icon { width: 90px; height: 42px; background: rgba(255,255,255,0.05); border-radius: 4px; display: flex; align-items: center; justify-content: center; font-size: 20px; }
            .caly-info { display: flex; flex-direction: column; gap: 2px; flex-grow: 1; }
            .caly-main-text { font-size: 15px; font-weight: 600; color: #e2e8f0; }
            .caly-sub-text { font-size: 12px; color: #94a3b8; font-family: monospace; opacity: 0.7; display: flex; align-items: center; gap: 5px; }
            .caly-actions { display: flex; gap: 8px; align-items: center; }
            .caly-btn { background: linear-gradient(135deg, #8b5cf6 0%, #6d28d9 100%); border: 1px solid #7c3aed; color: white; padding: 8px 16px; border-radius: 6px; font-weight: 600; cursor: pointer; font-size: 12px; transition: all 0.2s; display: flex; align-items: center; gap: 6px; text-transform: uppercase; letter-spacing: 0.5px; }
            .caly-btn:hover { transform: translateY(-2px); box-shadow: 0 4px 12px rgba(139, 92, 246, 0.4); filter: brightness(1.1); }
            .caly-icon-btn { background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); color: #94a3b8; padding: 8px; border-radius: 6px; cursor: pointer; transition: all 0.2s; display: flex; align-items: center; justify-content: center; }
            ...
            .caly-modal.success .caly-header { background: linear-gradient(90deg, #14532d, #13131a); border-bottom-color: #22c55e; }
    ...
    async function executeRename(folder, newName) {
        try {
            ...
            if (res.ok) { showRestoreModal(); } else { alert("Erro ao renomear."); }
    ...
    async function triggerRestore(folder, btnElement) {
        ...
        try {
            ...
            } else { alert("Erro no servidor."); btnElement.innerHTML = 'RESTAURAR'; btnElement.disabled = false; }
